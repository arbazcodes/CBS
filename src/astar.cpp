    ...
    while (!open_list.empty()) {
        ...
        for (size_t i = 0; i < directions.size(); ++i) {
            ...
            if (current.direction != new_direction) {
                State direction_change_state = {current.position, new_direction, current.time_step + 1};
                ...
                if (g_costs.find(direction_change_state) == g_costs.end() || 
                    g_cost_direction_change < g_costs[direction_change_state]) {
                    g_costs[direction_change_state] = g_cost_direction_change;
                    int f_cost_direction_change = g_cost_direction_change + 
                        ManhattanDistance(direction_change_state.position, goal);
